  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (showMenu && menuRef.current && !menuRef.current.contains(event.target)) {
        // This dismisses the logout confirmation toast if it's open
        toast.dismiss(); 
        onAvatarClick(); 
      }
    };
      ...
      {isSidebarOpen && (
        <div
          ...
          onClick={() => {
            ...
            toast.dismiss(); // Close toast if mobile overlay is clicked
          }}
        />
      )}

      {/* SIDEBAR */}
      <aside className={`fixed lg:sticky top-0 left-0 z-50 h-screen w-[280px] bg-white border-r border-[#EEEEEE] flex flex-col transition-transform duration-300 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        <div className="px-8 py-10 flex flex-col items-center border-b border-[#EEEEEE] relative">
          ...
          <h2 className="text-xs font-black text-[#5D4037] tracking-tight uppercase text-center px-2">{user?.full_name || "User"}</h2>
        ...
        <nav className="flex-1 px-4 py-6 space-y-2">
          {[{ name: "Dashboard", path: "/user/dashboard", icon: <LayoutDashboard size={18} /> },
            ...
            <div key={item.path} onClick={() => { navigate(item.path); setIsSidebarOpen(false); }} className={`flex items-center gap-4 px-6 py-4 rounded-[20px] text-[10px] font-black uppercase tracking-[1.5px] cursor-pointer transition-all ${location.pathname === item.path ? "bg-[#5D4037] text-white shadow-lg" : "text-gray-400 hover:bg-[#FAF6F3]"}`}>
              <span>{item.icon}</span>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="h-20 lg:h-24 px-6 lg:px-12 flex justify-between items-center bg-transparent">
          ...
          <div className="flex items-center gap-3 relative" ref={menuRef}>
            <div onClick={onAvatarClick} className="flex items-center gap-2 cursor-pointer bg-white p-1.5 pr-4 rounded-full border border-[#EEEEEE] shadow-sm hover:border-[#A67C52] transition-all">
              <img src={`${import.meta.env.VITE_IMG_URL}/photos/${user.photo}`} alt="user" className="w-8 h-8 rounded-full object-cover" />
              <span className="hidden sm:inline text-xs font-black text-[#5D4037] uppercase tracking-wider">{user?.full_name?.split(' ')[0] || "User"}</span>
            </div>
            ...
            {showMenu && (
              <div className="absolute right-0 top-16 w-52 bg-white rounded-[24px] shadow-2xl border border-[#EEEEEE] z-50 overflow-hidden p-2">
                <button onClick={() => { navigate("/user/dashboard/profile"); onAvatarClick(); }} className="w-full flex items-center gap-3 px-4 py-3 text-[10px] font-black uppercase tracking-widest text-[#5D4037] hover:bg-[#FAF6F3] rounded-xl">
                  <Settings size={16} /> Profile Settings
                </button>
                <button onClick={() => { handleLogout(); onAvatarClick(); }} className="w-full flex items-center gap-3 px-4 py-3 text-[10px] font-black uppercase tracking-widest text-rose-600 hover:bg-rose-50 rounded-xl">
        ...
        <main className="flex-1 px-4 lg:px-12 pb-6 lg:pb-10 overflow-y-auto">
          <div className="max-w-[1400px] mx-auto h-full">
            <div className="bg-white rounded-[32px] lg:rounded-[48px] p-6 lg:p-10 shadow-xl border border-[#EEEEEE] min-h-full">
              {children}
            </div>
          </div>
        </main>
