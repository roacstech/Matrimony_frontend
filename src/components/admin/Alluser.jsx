        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-8 md:mb-10 cursor-pointer">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5 md:gap-6 w-full lg:w-auto">
            ...
            <div className="flex items-center gap-4 md:gap-5">
              ...
              <div>
                <h2 className="text-xl md:text-2xl font-black text-[#5D4037] tracking-tight leading-tight">
          ...
          <div
            className={`flex items-center gap-3 px-5 py-3 rounded-2xl border-2 w-full lg:w-auto justify-center lg:justify-start ${selectedUser.is_active ? "bg-emerald-50 border-emerald-100 text-emerald-600" : "bg-rose-50 border-rose-100 text-rose-500 cursor-pointer"}`}
          >
        ...
        <table className="w-full text-left min-w-[700px]">
          ...
          <tbody className="divide-y divide-[#EEEEEE] ">
            {filteredUsers.map((u) => (
              <tr
                key={u.id}
                className="group hover:bg-[#FAF6F3]/50 transition-all cursor-pointer "
              >
                <td className="px-6 md:px-8 py-5">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-[13px] md:text-[14px] font-black text-[#5D4037] leading-none">
                        {u.fullName}
                      </p>
                      <p className="text-[9px] md:text-[10px] text-[#A67C52] font-bold uppercase tracking-widest mt-1.5">
                        {u.gender} • {u.city}
                      </p>
                    </div>
                  </div>
                ...
                <td className="px-6 md:px-8 py-5">
                  <p className="text-xs font-bold text-stone-600 truncate max-w-[150px]">
                    {u.occupation}
                  </p>
                  <p className="text-[10px] text-stone-400 uppercase font-black mt-1 tracking-tighter">
                    {u.income}
                  </p>
                </td>
  ...
  {u.privacy}
                ...
                <td className="px-6 md:px-8 py-5 text-center cursor-pointer">
                  <div className="flex flex-col items-center gap-1.5 cursor-pointer">
                    ...
                    >
                      <span
                        className={`absolute top-0.5 w-3.5 h-3.5 md:w-4 md:h-4 cursor-pointer bg-white rounded-full shadow-sm transition-all ${u.is_active ? "left-[19px] md:left-[22px]" : "left-0.5"}`}
                      />
                    </button>
                    <span
                      className={`text-[8px] font-black uppercase tracking-tighter ${u.is_active ? "text-[#A67C52]" : "text-rose-400 cursor"}`}
                    >
                ...
                <td className="px-6 md:px-8 py-5 text-center">
                  <button
                    ...
                    className="p-2.5 md:p-3 text-[#5D4037] bg-[#FAF6F3] hover:bg-[#5D4037] hover:text-white rounded-xl md:rounded-2xl transition-all shadow-sm cursor-pointer"
                  >
                    <Eye size={18} />
                  </button>
                </td>
...
const InfoBox = ({ label, value }) => (
  <div className="flex flex-col gap-1.5 group cursor-pointer">
    <label className="text-[8px] md:text-[9px] font-black text-stone-400 uppercase tracking-[1.5px] ml-1 group-hover:text-[#A67C52] transition-colors">
      {label}
    </label>
