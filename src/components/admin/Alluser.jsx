...
import { Eye, ArrowLeft, FileText, Download, Mail, MapPin, ShieldCheck, ShieldAlert, ExternalLink } from "lucide-react";
...
const AllUsers = () => {
  ...
  const filteredUsers = data.filter((u) => {
    ...
    if (filter === "male" || filter === "female") return u.gender.toLowerCase() === filter.toLowerCase();
    if (filter === "active") return u.isPublic === true;
    if (filter === "inactive") return u.isPublic === false;
    return true;
  });

  /* ================= 1. RESPONSIVE PROFILE DETAIL VIEW ================= */
  if (selectedUser) {
    return (
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-8 md:mb-10">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5 md:gap-6 w-full lg:w-auto">
            <button onClick={() => { setSelectedUser(null); setActiveTab("personal"); }} 
              className="p-3.5 md:p-4 bg-gray-50 rounded-[20px] md:rounded-[24px] hover:bg-[#3B1E54] hover:text-white transition-all shadow-sm">
              <ArrowLeft size={20} />
            </button>
            
            <div className="flex items-center gap-4 md:gap-5">
              <div className="w-16 h-16 md:w-20 md:h-20 rounded-[22px] md:rounded-[28px] bg-[#3B1E54] overflow-hidden border-4 border-[#F3F0F7] shadow-md flex items-center justify-center flex-shrink-0">
                {selectedUser.photo ? (
              ...
              <div>
                <h2 className="text-xl md:text-2xl font-black text-[#3B1E54] tracking-tight leading-tight">{selectedUser.fullName}</h2>
                <div className="flex flex-col gap-0.5 mt-1">
                  <p className="flex items-center gap-1.5 text-[10px] font-black text-[#9B7EBD] uppercase tracking-wider">
        ...
        <div className="flex gap-1 mb-8 overflow-x-auto no-scrollbar border-b border-gray-100 -mx-5 px-5 md:mx-0 md:px-0">
          {["personal", "education", "family", "horoscope"].map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`px-6 md:px-8 py-3 md:py-4 rounded-t-[15px] md:rounded-t-[20px] text-[10px] md:text-[11px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === tab ? "bg-[#3B1E54] text-white shadow-lg" : "text-gray-400 hover:text-[#3B1E54]"}`}>
              {tab}
            </button>
          ))}
        </div>
          ...
          {activeTab === "horoscope" && (
            <>
              ...
              <div className="sm:col-span-2 lg:col-span-3 mt-4">
                <div className="p-5 md:p-6 bg-[#FBFBFE] border border-gray-100 rounded-3xl flex flex-col md:flex-row items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    <div className="p-3 bg-white rounded-2xl shadow-sm text-[#3B1E54] flex-shrink-0">
                      <FileText size={24} />
                    </div>
                    <div>
                      <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Jadhagam File</p>
                      <p className="text-sm font-bold text-[#3B1E54] truncate max-w-[150px] sm:max-w-xs">{selectedUser.horoscope?.uploaded ? (selectedUser.horoscope.fileName || "horoscope.pdf") : "Not Uploaded"}</p>
                    </div>
                  </div>
      ...
      <a
          ...
          className="px-6 py-2.5 bg-[#3B1E54] text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:opacity-90 transition"
  ...
  return (
    <div className="bg-white rounded-[30px] md:rounded-[40px] shadow-sm border border-gray-100 overflow-hidden">
      ...
      <div className="p-6 md:p-8 border-b border-gray-50 flex flex-col xl:flex-row justify-between items-center gap-6">
        <div className="text-center xl:text-left">
          <h2 className="text-xl md:text-2xl font-black text-[#3B1E54] tracking-tight">User Records</h2>
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">Manage profiles and visibility</p>
        </div>
        <div className="flex bg-gray-100/50 p-1.5 rounded-[22px] border border-gray-100 overflow-x-auto no-scrollbar max-w-full">
          {["all", "male", "female", "active", "inactive"].map((f) => (
            <button key={f} onClick={() => setFilter(f)} 
              className={`px-5 md:px-6 py-2 md:py-2.5 text-[9px] md:text-[10px] font-black uppercase rounded-[18px] transition-all whitespace-nowrap ${filter === f ? "bg-white text-[#3B1E54] shadow-sm" : "text-gray-400 hover:text-gray-600"}`}>
              {f}
            </button>
          ))}
        </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left min-w-[700px]">
          ...
          <tbody className="divide-y divide-gray-50">
            {filteredUsers.map((u) => (
              <tr key={u.id} className="group hover:bg-[#F9F7FC]/50 transition-all">
                <td className="px-6 md:px-8 py-5">
                  <div className="flex items-center gap-4">
                    <div className="w-11 h-11 md:w-12 md:h-12 rounded-[16px] md:rounded-[18px] bg-[#3B1E54] overflow-hidden flex-shrink-0 flex items-center justify-center">
                    ...
                    <div>
                      <p className="text-[13px] md:text-[14px] font-black text-gray-800 leading-none">{u.fullName}</p>
                      <p className="text-[9px] md:text-[10px] text-[#9B7EBD] font-bold uppercase tracking-widest mt-1.5">{u.gender} • {u.city}</p>
                    </div>
                  </div>
                </td>
                <td className="px-6 md:px-8 py-5">
                ...
                <td className="px-6 md:px-8 py-5 text-center">
                  <div className="flex flex-col items-center gap-1.5">
                    <button onClick={() => togglePublicStatus(u.id)} className={`w-9 h-4.5 md:w-10 md:h-5 rounded-full relative transition-all ${u.isPublic ? "bg-emerald-500 shadow-inner" : "bg-gray-200"}`}>
                    ...
                    <span className={`text-[8px] font-black uppercase tracking-tighter ${u.isPublic ? 'text-emerald-600' : 'text-rose-400'}`}>
                      {u.isPublic ? 'ACTIVE' : 'INACTIVE'}
                    </span>
                  </div>
                </td>
                <td className="px-6 md:px-8 py-5 text-center">
                  <button onClick={() => setSelectedUser(u)} className="p-2.5 md:p-3 text-[#3B1E54] bg-gray-50 hover:bg-[#3B1E54] hover:text-white rounded-xl md:rounded-2xl transition-all">
                    <Eye size={18} />
                  </button>
                </td>
              </tr>
      ...
      <div className="md:hidden p-4 bg-gray-50 text-center">
        <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Swipe left to see more columns →</p>
      </div>
    </div>
  );
};
...
const InfoBox = ({ label, value }) => (
  <div className="flex flex-col gap-1.5 group">
    <label className="text-[8px] md:text-[9px] font-black text-gray-400 uppercase tracking-[1.5px] ml-1 group-hover:text-[#9B7EBD] transition-colors">{label}</label>
    <div className="px-5 md:px-6 py-4 md:py-5 bg-gray-50 border border-gray-100 rounded-[20px] md:rounded-[24px] text-[12px] md:text-[13px] font-bold text-[#3B1E54] group-hover:border-[#D4BEE4] group-hover:bg-white transition-all shadow-sm">
      {value || "—"}
    </div>
  </div>
);
